...
namespace pan{
	TEST_F(OutbreakTest, Test1)
	{
		...
		ASSERT_EQ(game.map[inf.city].getCubes(inf.diseaseType), inf.cubes);
		...
		ASSERT_EQ(game.gameData.diseaseCubes[inf.diseaseType], s.diseaseCubesPerDisease - 6);
		ASSERT_EQ(game.map[0].getCubes(inf.diseaseType), 3);
		ASSERT_EQ(game.map[1].getCubes(inf.diseaseType), 3);

		// Further outbreaks cause an increase of marker by two but do not decrease the number
		// of disease cubes
		auto cubesCount = game.gameData.diseaseCubes[inf.diseaseType];
		ASSERT_TRUE(inf.execute(game.actionHandler));
		ASSERT_EQ(game.gameData.outbreakMarker, 3);
		ASSERT_EQ(game.gameData.diseaseCubes[inf.diseaseType], cubesCount);
	}
	...
	TEST_F(OutbreakTest, Test2)
	{
		...
		for (std::size_t i = 0; i < n; i++){
			...
			ASSERT_TRUE(inf.execute(game.actionHandler));
			ASSERT_EQ(game.gameData.outbreakMarker, 0);
			ASSERT_EQ(game.map[i].getCubes(inf.diseaseType), inf.cubes);
		}

		// Will cause a chain outbreak, all n cities will suffer and outbreak
		auto cubesSize = game.gameData.diseaseCubes[inf.diseaseType];
		ASSERT_TRUE(inf.execute(game.actionHandler));
		ASSERT_EQ(game.gameData.outbreakMarker, n);
		// The diseases cubes will decrease by n
		ASSERT_EQ(game.gameData.diseaseCubes[inf.diseaseType], cubesSize);
	}
}